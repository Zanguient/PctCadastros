...
begin
  ...
  Op.DesabilitaCampos(FrmCadastro_Grupo_Produto);
...
begin
  ...
  begin
    FGrupoProdutoDominio:= TGrupoProdutoDominio.Create(Conexao, FGrupoProduto);
...
begin
  ...
  begin
    ...
    if (achei = false) then
    begin
      EdtCodigo.Text:= IntToStr(GeraCodigo.GeraCodigoSequencia('Cadastro_Grupo_Produtos', Conexao));
...
procedure TFrmCadastro_Grupo_Produto.IniciaTela;
begin
  BBtnSalvar.Enabled:= false;
  BBtnNovo.Enabled:= true;
  BBtnCancelar.Enabled:= false;
...
var
  Retorno: AnsiString;
begin
  FGrupoProdutoDominio:= TGrupoProdutoDominio.Create(Conexao);
