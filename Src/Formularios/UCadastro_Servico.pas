...
begin
  ...
  Op.DesabilitaCampos(FrmCadastro_Servico);
...
begin
  ...
  begin
    FServicoDominio:= TServicoDominio.Create(Conexao, FServico);
...
begin
  ...
  begin
    ...
    if (achei = false) then
    begin
      EdtCodigo.Text:= IntToStr(GeraCodigo.GeraCodigoSequencia('Cadastro_Servico', Conexao));
...
procedure TFrmCadastro_Servico.IniciaTela;
begin
  BBtnSalvar.Enabled:= false;
  BBtnNovo.Enabled:= true;
  BBtnCancelar.Enabled:= false;
...
var
  Retorno: AnsiString;
begin
  FServicoDominio:= TServicoDominio.Create(Conexao);
