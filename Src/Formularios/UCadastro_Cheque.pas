...
begin
  ...
  Op.DesabilitaCampos(FrmCadastro_Cheque);
...
begin
  ...
  begin
    FChequeDominio:= TChequeDominio.Create(Conexao, FCheque);
...
begin
  ...
  TOperacoesConexao.IniciaQuerys([qryConsulta, DM.qryContaBancaria], Conexao);
...
begin
  ...
  begin
    ...
    if (achei = false) then
    begin
      EdtCodigo.Text:= IntToStr(GeraCodigo.GeraCodigoSequencia('Cadastro_Cheque', Conexao));
...
procedure TFrmCadastro_Cheque.IniciaTela;
begin
  BBtnSalvar.Enabled:= false;
  BBtnNovo.Enabled:= true;
  BBtnCancelar.Enabled:= false;
