...
uses
  ...
  HistoricoEntidade, PluviometroDominio, PluviometroEntidade;

type
  TFrmCadastro_Pluviometro = class(TForm)
  ...
  private
    ...
    FEntidadeDominio: TPluviometroDominio;
...
var
  FrmCadastro_Pluviometro: TFrmCadastro_Pluviometro;
implementation
...
procedure TFrmCadastro_Pluviometro.BBtnCancelarClick(Sender: TObject);
begin
  Op.LimpaCampos(FrmCadastro_Pluviometro);
  Op.DesabilitaEdits(FrmCadastro_Pluviometro);
...
procedure TFrmCadastro_Pluviometro.BBtnExcluirClick(Sender: TObject);
var
  Retorno: AnsiString;
begin
  if (Mensagens.MensagemConfirmacao(MensagemConfirmaExclusao)) then
  begin
    FEntidadeDominio:= TPluviometroDominio.Create(dm.ADOConnection1, FEntidade);
...
procedure TFrmCadastro_Pluviometro.BBtnFecharClick(Sender: TObject);
begin
...
procedure TFrmCadastro_Pluviometro.BBtnNovoClick(Sender: TObject);
begin
  PageControl1.TabIndex:= 0;
  Op.HabilitaEdits(FrmCadastro_Pluviometro);
  Op.LimpaCampos(FrmCadastro_Pluviometro);
...
procedure TFrmCadastro_Pluviometro.BBtnSalvarClick(Sender: TObject);
var
  Retorno: AnsiString;
begin
  if (Confira = true) then
  begin
    FEntidade:= TPluviometroEntidade.Create;
    ...
    if (achei = false) then
    begin
      EdtCodigo.Text:= IntToStr(GeraCodigo.GeraCodigoSequencia('Cadastro_Pluviometro', dm.ADOConnection1));
...
procedure TFrmCadastro_Pluviometro.BuscaDados;
var
  Retorno: AnsiString;
begin
  FEntidadeDominio:= TPluviometroDominio.Create(dm.ADOConnection1);
...
function TFrmCadastro_Pluviometro.Confira: boolean;
begin
  Confira:= false;
...
constructor TFrmCadastro_Pluviometro.Create(FPropriedade: TPropriedadeEntidade; FUsuario: TLoginEntidade);
begin
  Self.FPropriedade:= FPropriedade;
  Self.FUsuario:= FUsuario;
end;

procedure TFrmCadastro_Pluviometro.cxGrid1DBTableView1DblClick(Sender: TObject);
begin
  PageControl1.TabIndex:= 0;
  achei:= true;
  Op.HabilitaEdits(FrmCadastro_Pluviometro);
...
procedure TFrmCadastro_Pluviometro.EdtCidadeKeyPress(Sender: TObject;
  var Key: Char);
begin
  Op.SomenteApaga(key);
end;

procedure TFrmCadastro_Pluviometro.EdtCodigo_CidadeKeyPress(Sender: TObject;
  var Key: Char);
begin
  Op.SomenteApaga(key);
end;

procedure TFrmCadastro_Pluviometro.EdtSetorKeyPress(Sender: TObject; var Key: Char);
begin
  Op.SomenteApaga(key);
end;

procedure TFrmCadastro_Pluviometro.FormActivate(Sender: TObject);
begin
  ativo:= true;
end;

procedure TFrmCadastro_Pluviometro.FormCloseQuery(Sender: TObject;
...
begin
  if (BBtnCancelar.Enabled = true) then
    ...
    begin
      FrmCadastro_Pluviometro.Free;
      FrmCadastro_Pluviometro:= Nil;
    end;
end;

procedure TFrmCadastro_Pluviometro.FormDeactivate(Sender: TObject);
begin
  ativo:= false;
end;

procedure TFrmCadastro_Pluviometro.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmCadastro_Pluviometro.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  begin
...
procedure TFrmCadastro_Pluviometro.FormShow(Sender: TObject);
var
  Retorno: AnsiString;
begin
  PageControl1.TabIndex:= 0;
  Op.HabilitaCampos(FrmCadastro_Pluviometro);
  Op.LimpaCampos(FrmCadastro_Pluviometro);
  Op.DesabilitaEdits(FrmCadastro_Pluviometro);
end;

procedure TFrmCadastro_Pluviometro.MEdtData_CadastroEnter(Sender: TObject);
begin
  MEdtData_Cadastro.Text:= DateTimeToStr(now);
end;

procedure TFrmCadastro_Pluviometro.MEdtData_CadastroExit(Sender: TObject);
begin
...
{initialization
  RegisterClass(TFrmCadastro_Pluviometro);

finalization
  UnRegisterClass(TFrmCadastro_Pluviometro);}
